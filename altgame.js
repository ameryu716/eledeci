"use strict";
function Fadeout(elemen) {
    elemen.style.display = "none";
}
function Fadein(elemen) {
    elemen.style.display = "block";
    elemen.style.opacity = "1";
}
function DelayFadein(delay, ...node) {
    if (delay < 10) {
...
function DelayFlexin(delay, ...node) {
    if (delay < 10) {
        for (let i = 0; i < node.length; i++) {
...
function DelayFadeout(delay, ...node) {
    if (delay < 10) {
        for (let i = 0; i < node.length; i++) {
            node[i].style.transition = "0";
            node[i].style.opacity = "1";
...
window.addEventListener("load", () => {
    ...
    const hyoukalist = {
        h_heibon: ["高みを目指して！", "もうちょっとで頂上", "惜しい！あと少しです。", "あとちょっと！！"],
        h_bonjin: ["そんなに褒めてほしいのか", "ま、良いんじゃね？", "レベル・凡人", "まだまだいけるよ"],
        h_manten: ["君は神かもしれない", "君に教えることはもうない", "その力で世界を救え", "レベル・神"],
        h_gomten: ["馬鹿なの？（直球）", "何故生きている！？", "えぇ.....（困惑）", "大丈夫ですか？（煽り）"],
