function Fadeout(elemen: HTMLElement): void {
	elemen.style.display = "none";
}
function Fadein(elemen: HTMLElement): void {
	elemen.style.display = "block";
	elemen.style.opacity = "1";
}

function DelayFadein(delay: number, ...node: HTMLElement[]): void {
	if (delay < 10) {
...
function DelayFlexin(delay: number, ...node: HTMLElement[]): void {
	if (delay < 10) {
		for (let i = 0; i < node.length; i++) {
...
function DelayFadeout(delay: number, ...node: HTMLElement[]): void {
	if (delay < 10) {
		for (let i = 0; i < node.length; i++) {
			node[i].style.transition = "0";
			node[i].style.opacity = "1";
...
let clickflag: boolean = true;　//多重処理防止
let canScroll:boolean = false; //スコア表スクロール可能譜

// let seikaisuu: number = 0;　//一次正解数
// let huseikaisuu: number = 0;　//一次不正解数
// let kotaetakazu: number = 0;　//一次出題数

let totalok: number = 0;　//総合正解数
let totalng: number = 0;　//総合不正解数
let totalQuestions: number = 0;　//総合出題数
let totalratec: number = 0; //総合正解率

let scoreComment: string = ""; //評価コメント（ランダム）
let ratecorrect: number = 0;  //正解率
let unlimitedcontinue: boolean = true;  //50問以上で表示・続けるかどうか

window.addEventListener("load", () => {

	const hyouka: HTMLDivElement = (<HTMLDivElement>document.getElementById("hyouka"));
	const gamecorner: HTMLDivElement = (<HTMLDivElement>document.getElementById("game-corner"));
	const optiondiv: HTMLDivElement = <HTMLDivElement>document.getElementById("option");
	const startdiv: HTMLDivElement = <HTMLDivElement>document.getElementById("startdiv");

	const clearop: HTMLDivElement = <HTMLDivElement>document.getElementById("clearop");
	const scoreParents: HTMLDivElement = <HTMLDivElement>document.getElementById("Score");
	let scorenode: HTMLUListElement = <HTMLUListElement>document.getElementsByClassName("nowrails")[0];
	const railPages:HTMLUListElement = <HTMLUListElement>document.getElementById("railPages");
	const gamestartbtn: HTMLButtonElement = <HTMLButtonElement>document.getElementById("gamestartbtn");
	const missiontitle: HTMLHeadingElement = <HTMLHeadingElement>document.getElementById("missiontitle");//問題文elements
	const selectans1: HTMLDivElement = <HTMLDivElement>document.getElementById("selectans-1");//選択肢1elements
	const selectans2: HTMLDivElement = <HTMLDivElement>document.getElementById("selectans-2");//選択肢2elements
	const missionnumberElement: HTMLSpanElement = <HTMLSpanElement>document.getElementById("mission-number");//何問目表示

	const backbtn: HTMLSpanElement = <HTMLSpanElement>document.getElementById("back");
	const navmessage: HTMLSpanElement = <HTMLSpanElement>document.getElementById("top-message");
	const alscore1: HTMLSpanElement = <HTMLSpanElement>document.getElementById("alsc1");
	const alscore2: HTMLSpanElement = <HTMLSpanElement>document.getElementById("alsc2");

	const heximg: HTMLImageElement = <HTMLImageElement>document.getElementById("heximg");//青六角形
	const greenhex: HTMLImageElement = <HTMLImageElement>document.getElementById("greenhex");//緑六角形
	let railPagesCount: number = 11;//評価リストタイトル用
	let scroller:number = 50;//ページング用トランスフォーム
	let pagingVolume:number =190; //デバイスごとのスクロール量
	let sumahoflag:boolean = false;//スマホフラグ
	let pxORper:string = "px";//デバイスごと単位管理
	...
	const Erabu: Array<Array<string | number>> = [
		['星の"奔流"', "ほんりゅう", "はんりゅう", 1],
		['軍隊"行脚"', "あんどん", "あんぎゃ", 2],
	...
	const hyoukalist: hyoukalisttype = {
	...
	function hyoukafase(): void {
		ratecorrect = 0;
		let randomhyouka: 0 | 1 | 2 | 3 = (<0 | 1 | 2 | 3>Math.floor(Math.random() * 4));
		scoreComment = "";
	...
	function resetsur(): void {
		const whatMissionNum = Math.floor(Math.random() * 227) + 1;//乱数から問セッティング227問
		missiontitle.innerText = <string>Erabu[whatMissionNum][0];
		Gamestate.rightnum = <1 | 2>Erabu[whatMissionNum][3];
		selectans1.innerText = <string>Erabu[whatMissionNum][1];
		selectans2.innerText = <string>Erabu[whatMissionNum][2];
		missionnumberElement.innerText = "第" + (Gamestate.wq + 1) + "問";
		//問題格納
	}

	function modechange() {
		const modecheck: NodeListOf<HTMLInputElement> = (<NodeListOf<HTMLInputElement>>document.getElementsByName("limitchange"));
	...
	gamestartbtn.addEventListener("mouseleave", () => {
		DelayFadeout(0.5, heximg, greenhex);
	})

	//正解エフェクト不正解エフェクト
	const okimg: HTMLImageElement = <HTMLImageElement>document.getElementById("okimg");//〇
	const okaud: HTMLAudioElement = <HTMLAudioElement>document.getElementById("okaud");//ピンポーン
	const gameobrn: HTMLVideoElement = <HTMLVideoElement>document.getElementById("gameobrn");//デレレーンゲームオーバー
	const ngbrn: HTMLVideoElement = <HTMLVideoElement>document.getElementById("ngbrn");//バーン！！動画
	const audbn: HTMLAudioElement = <HTMLAudioElement>document.getElementById("audbn");//バーン！！音声
	...
	function changeRails(){
		...
		scorenode = <HTMLUListElement>document.getElementsByClassName("nowrails")[0];
	}

	function scoreScroll(direc:"right"|"left"){
	...
	function tailSelect(tailnum:1|2){
		if (clickflag) {
			clickflag = false;
